...
def load_image_file_crop(img_path: str, ratio=1.0, target_size=518):
    ...
    """
    # Check for JPEG, JPG, WEBP
    if img_path.lower().endswith(('.jpg', '.jpeg', '.webp')):
        ...
        # If the image has an alpha channel (PNG), remove it by taking the first 3 channels (RGBA -> RGB)
        if img.shape[-1] == 4:  # if there's an alpha channel (RGBA)
            img = img[..., :3]  # Remove the alpha channel and keep RGB
